...
public class AtmCardActivity extends AppCompatActivity {
    ...
    private void createOrderAndProcessPayment() {
        ...
        // Parse price từ String sang Integer
        ...
        // Tạo OrderRequest
        String shippingAddress = "Chưa cập nhật";
        String phone = "Chưa cập nhật";
        OrderRequest orderRequest = new OrderRequest(items, "atm", shippingAddress, phone);
            ...
            public void onResponse(Call<BaseResponse<OrderResponse>> call, Response<BaseResponse<OrderResponse>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (body.getSuccess() && body.getData() != null) {
                        ...
                        Toast.makeText(AtmCardActivity.this, errorMsg, Toast.LENGTH_SHORT).show();
                    }
                } else {
                    progressDialog.dismiss();
                    String errorMsg = NetworkUtils.extractErrorMessage(response);
                    Toast.makeText(AtmCardActivity.this, "Lỗi: " + errorMsg, Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onResponse(Call<BaseResponse<CreatePaymentResponse>> call, Response<BaseResponse<CreatePaymentResponse>> response) {
                if (response.isSuccessful() && response.body() != null && response.body().getSuccess()) {
                    ...
                    if (data != null && data.getPaymentId() != null) {
                        processAtmPayment(data.getPaymentId());
                    } else {
                        progressDialog.dismiss();
                        Toast.makeText(AtmCardActivity.this, "Không lấy được paymentId", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    progressDialog.dismiss();
                    String errorMsg = NetworkUtils.extractErrorMessage(response);
                    Toast.makeText(AtmCardActivity.this, "Lỗi tạo thanh toán: " + errorMsg, Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void processAtmPayment(String paymentId) {
        ...
        // Ở backend ATM đang dùng atmBankCode/atmAccountNumber, ở đây tạm map đơn giản:
        ...
        // Dùng lại processCreditCard để không phải tạo thêm endpoint ATM riêng trên app
